...
@section('title', 'Edit Jenis Layanan Surat')
...
<div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem;">
    ...
    <h2 style="margin: 0;">Edit Jenis Layanan Surat</h2>
</div>
...
<div class="glass" style="padding: 2rem; border-radius: 16px;">
    <form action="{{ route('jenis-surat.update', $jenisSurat->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" id="nama_layanan" name="nama_layanan" value="{{ old('nama_layanan', $jenisSurat->nama_layanan) }}" placeholder="Contoh: Surat Keterangan Tidak Mampu" class="glass-select" style="width: 100%;" required>
            @error('nama_layanan')
        ...
        <div class="form-group">
            ...
            <input type="text" id="kode_surat" name="kode_surat" value="{{ old('kode_surat', $jenisSurat->kode_surat) }}" placeholder="Contoh: SKT, SKM, SKU" class="glass-select" style="width: 100%;">
            <small style="color: var(--text-muted); font-size: 0.8rem;">Kode singkat untuk auto-generate nomor surat (maks 20 karakter)</small>
            @error('kode_surat')
                <span class="error-text">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="deskripsi" name="deskripsi" class="glass-select" rows="3" style="width: 100%; height: auto;" placeholder="Deskripsi singkat tentang jenis layanan ini...">{{ old('deskripsi', $jenisSurat->deskripsi) }}</textarea>
            @error('deskripsi')
                <span class="error-text">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="persyaratan" name="persyaratan" class="glass-select" rows="3" style="width: 100%; height: auto;" placeholder="Persyaratan yang diperlukan untuk layanan ini...">{{ old('persyaratan', $jenisSurat->persyaratan) }}</textarea>
            @error('persyaratan')
                <span class="error-text">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" id="template_file" name="template_file" value="{{ old('template_file', $jenisSurat->template_file) }}" placeholder="Contoh: surat.penghasilan" class="glass-select" style="width: 100%;">
        ...
        <input type="hidden" name="form_template" id="form_template" value="{{ old('form_template', $jenisSurat->form_template ?? '') }}">
...
[
  ...
  {"name": "penghasilan_sampai", "label": "Penghasilan Sampai", "type": "number", "required": true}
]'>{{ old('form_fields', $jenisSurat->form_fields ? json_encode($jenisSurat->form_fields, JSON_PRETTY_PRINT) : '') }}</textarea>
        ...
        <div class="form-group">
            ...
            <input type="hidden" name="template_id" id="template_id" value="{{ old('template_id', $jenisSurat->template_id) }}">

            @if($templates->count() > 0)
                <div class="templates-grid">
                    {{-- Opsi tanpa template --}}
                    @php $currentTemplateId = old('template_id', $jenisSurat->template_id); @endphp
                    <div class="template-card {{ empty($currentTemplateId) ? 'selected' : '' }}" onclick="selectTemplate(this, '', '', '')">
                    ...
                    <div class="template-card {{ $currentTemplateId == $template->id ? 'selected' : '' }}" onclick="selectTemplate(this, '{{ $template->id }}', '{{ $template->form_template ?? '' }}', '{{ $template->cetak_template ?? '' }}')">
                        <div class="check-icon"><i class="ri-check-line"></i></div>
