...
@section('title', 'Edit Perangkat - Admin Desa')
...
<div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem;">
    <a href="{{ route('perangkat.index') }}" class="glass-select" style="background: transparent; color: var(--text-main); border: 1px solid var(--text-muted); padding: 0.5rem 1rem; font-weight: 500; text-decoration: none; display: inline-flex; align-items: center; gap: 0.4rem;">
    ...
    <form action="{{ route('perangkat.update', $perangkat->id) }}" method="POST" enctype="multipart/form-data">
       ...
       <div class="form-group">
            ...
            <select name="penduduk_id" id="penduduk_select" class="glass-select" style="width: 100%;">
                ...
                <option value="{{ $data->id }}" data-nama="{{ strtolower($data->nama_lengkap) }}" data-nik="{{ strtolower($data->nik ?? '') }}" {{ $perangkat->penduduk_id == $data->id ? 'selected' : '' }}>
                ...
                @endforeach
                    ...
                    <label for="nama_perangkat">Nama Perangkat <span class="required">*</span></label>
                        <select name="jabatan_id" class="glass-select" style="width: 100%;">
                            ...
                            <option value="{{ $data->id }}" {{ $perangkat->jabatan_id == $data->id ? 'selected' : '' }}>
                                {{ $data->nama_jabatan }}
                            </option>
                            @endforeach
                        </select>
            ...
            <div class="col-6">
                <div class="form-group">
                    ...
                    <input type="text" id="profil" name="profil" value="{{ $perangkat->profil }}" class="glass-select" style="width: 100%;">
                    @error('profil')
                        <span class="error-text">{{ $message }}</span>
                    @enderror
                </div>
...
function filterPenduduk() {
    ...
    for (var i = 0; i < options.length; i++) {
        ...
        if (i === 0 || i === 1) { // header/empty option
