...
@section('konten')
<h2 style="margin-bottom: 2rem;">Tambah Pengumuman</h2>
...
<div class="glass" style="padding: 2rem; border-radius: 16px;">
    ...
    <form method="POST" action="{{ route('pengumuman.store') }}">
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="judul" value="{{ old('judul') }}" class="glass-select" style="width:100%;" required>
        </div>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <textarea name="isi" class="glass-select" rows="6" style="width:100%; height:auto;" required>{{ old('isi') }}</textarea>
        </div>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <label class="radio-container">
                <input type="radio" name="tipe" value="umum" class="radio-input" {{ old('tipe', 'umum') == 'umum' ? 'checked' : '' }} onchange="toggleTarget()">
                Umum (Semua Warga)
            </label>
            <label class="radio-container">
                <input type="radio" name="tipe" value="khusus" class="radio-input" {{ old('tipe') == 'khusus' ? 'checked' : '' }} onchange="toggleTarget()">
                Khusus (Warga Tertentu)
            </label>
        </div>

        <div id="target-warga-field" style="margin-bottom: 1.5rem; {{ old('tipe') == 'khusus' ? '' : 'display:none;' }}">
            ...
            <select name="target_user_id" class="glass-select" style="width:100%;">
                ...
                @foreach($warga as $w)
                <option value="{{ $w->id }}" {{ old('target_user_id') == $w->id ? 'selected' : '' }}>
                    {{ $w->name }} ({{ $w->nik ?? '-' }})
                </option>
                @endforeach
            </select>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="datetime-local" name="published_at" value="{{ old('published_at') }}" class="glass-select" style="width:100%;">
        </div>

        <div style="margin-top: 2rem; text-align: right;">
            <a href="{{ route('pengumuman.index') }}" class="glass-select" style="padding: 0.8rem 1.5rem; text-decoration:none; display:inline-flex; align-items:center; gap:.5rem; margin-right: 0.5rem;">
