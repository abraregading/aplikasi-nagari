...
@section('title', 'Tambah Posyandu - Admin Desa')
...
<div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem;">
    ...
    <h2 style="margin: 0;">Tambah Posyandu</h2>
</div>
    ...
    <form action="{{ route('posyandu.store') }}" method="POST">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div class="form-group">
                ...
                <input type="text" id="kode_posyandu" name="kode_posyandu" value="{{ old('kode_posyandu') }}" placeholder="Contoh: PY-001" class="glass-select" style="width: 100%;" required>
                @error('kode_posyandu')
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama_posyandu" name="nama_posyandu" value="{{ old('nama_posyandu') }}" placeholder="Contoh: Posyandu Mawar" class="glass-select" style="width: 100%;" required>
                @error('nama_posyandu')
                    <span class="error-text">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="jorong" name="jorong" value="{{ old('jorong') }}" placeholder="Contoh: Jorong Koto" class="glass-select" style="width: 100%;">
                @error('jorong')
                    <span class="error-text">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-group">
            ...
            <textarea id="alamat" name="alamat" class="glass-select" style="width: 100%; min-height: 80px;" placeholder="Masukkan alamat lengkap">{{ old('alamat') }}</textarea>
            @error('alamat')
                <span class="error-text">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="deskripsi" name="deskripsi" class="glass-select" style="width: 100%; min-height: 80px;" placeholder="Deskripsi singkat tentang pos yandu">{{ old('deskripsi') }}</textarea>
            @error('deskripsi')
                <span class="error-text">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="kader-row">
            ...
            <div class="form-group" style="margin-bottom: 0;">
                ...
                <input type="text" id="kaders_{{ $i }}_nama_kader" name="kaders[{{ $i }}][nama_kader]" value="{{ old('kaders.' . $i . '.nama_kader') }}" placeholder="Nama lengkap" class="glass-select" style="width: 100%;">
            </div>
            <div class="form-group" style="margin-bottom: 0;">
                <label for="kaders_{{ $i }}_jabatan">Jabatan</label>
                <input type="text" id="kaders_{{ $i }}_jabatan" name="kaders[{{ $i }}][jabatan]" value="{{ old('kaders.' . $i . '.jabatan') }}" placeholder="Contoh: Ketua, Anggota" class="glass-select" style="width: 100%;">
            </div>
            <div class="form-group" style="margin-bottom: 0;">
                <label for="kaders_{{ $i }}_no_hp">No. HP</label>
                <input type="text" id="kaders_{{ $i }}_no_hp" name="kaders[{{ $i }}][no_hp]" value="{{ old('kaders.' . $i . '.no_hp') }}" placeholder="Contoh: 0812xxxx" class="glass-select" style="width: 100%;">
            </div>
        </div>
        @endfor
