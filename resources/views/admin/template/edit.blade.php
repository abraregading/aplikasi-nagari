...
<div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem;">
    ...
    <h2 style="margin: 0;">Edit Template Surat</h2>
</div>
...
<div class="glass" style="padding: 2rem; border-radius: 16px;">
    <form action="{{ route('template-surat.update', $template->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" id="nama_template" name="nama_template" value="{{ old('nama_template', $template->nama_template) }}" placeholder="Contoh: Surat Keterangan Umum" class="glass-select" style="width: 100%;" required>
            @error('nama_template')
            ...
            <div class="form-group">
                ...
                <div style="position: relative;">
                    <select id="tipe" name="tipe" class="glass-select" style="width: 100%;">
                        ...
                        <option value="keterangan" {{ old('tipe', $template->tipe) == 'keterangan' ? 'selected' : '' }}>Surat Keterangan</option>
                        <option value="pengantar" {{ old('tipe', $template->tipe) == 'pengantar' ? 'selected' : '' }}>Surat Pengantar</option>
                        <option value="pernyataan" {{ old('tipe', $template->tipe) == 'pernyataan' ? 'selected' : '' }}>Surat Pernyataan</option>
                        <option value="rekomendasi" {{ old('tipe', $template->tipe) == 'rekomendasi' ? 'selected' : '' }}>Surat Rekomendasi</option>
                        <option value="undangan" {{ old('tipe', $template->tipe) == 'undangan' ? 'selected' : '' }}>Surat Undangan</option>
                        <option value="lainnya" {{ old('tipe', $template->tipe) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
        ...
        <div class="form-group">
            ...
            <textarea id="deskripsi" name="deskripsi" class="glass-select" rows="3" style="width: 100%; height: auto;" placeholder="Deskripsi singkat tentang template surat ini...">{{ old('deskripsi', $template->deskripsi) }}</textarea>
            @error('deskripsi')
                <span class="error-text">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div class="form-group">
                ...
                <select id="form_template" name="form_template" class="glass-select" style="width: 100%;">
                    ...
                    <option value="operator.buatsurat.forms.surat-penghasilan" {{ old('form_template', $template->form_template) == 'operator.buatsurat.forms.surat-penghasilan' ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select id="cetak_template" name="cetak_template" class="glass-select" style="width: 100%;">
                    ...
                    <option value="surat.penghasilan" {{ old('cetak_template', $template->cetak_template) == 'surat.penghasilan' ? 'selected' : '' }}>
                        Surat Penghasilan
                    </option>
                </select>
        ...
        <div class="form-group">
            ...
            <div style="background: rgba(255,255,255,0.05); border: 1px solid var(--border-glass); border-radius: 12px; overflow: hidden;">
                <textarea id="isi_template" name="isi_template" rows="20" style="width: 100%; min-height: 400px; padding: 1rem; background: transparent; border: none; color: var(--text-main); font-family: 'Courier New', monospace; font-size: 0.9rem; resize: vertical; outline: none;">{{ old('isi_template', $template->isi_template) }}</textarea>
            </div>
