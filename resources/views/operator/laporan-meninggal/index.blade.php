...
<div class="glass" style="padding: 2rem; border-radius: 16px; margin-bottom: 1.5rem;">
    <form method="GET" action="{{ route('operator.laporan-meninggal.index') }}" style="display: flex; gap: 1rem; align-items: flex-end; flex-wrap: wrap;">
        ...
        <a href="{{ route('operator.laporan-meninggal.cetak', ['bulan' => $bulan, 'tahun' => $tahun, 'jorong' => $jorongFilter]) }}" target="_blank" class="glass-select" style="background: #10b981; color: white; border: none; padding: 0.5rem 1.5rem; border-radius: 8px; text-decoration: none; display: inline-flex; align-items: center; gap: 0.3rem;">
...
<div class="glass" style="padding: 2rem; border-radius: 16px;">
    ...
    @if($dataMeninggal->count() > 0)
        @if($jorongFilter)
            @include('operator.laporan-meninggal._table', ['dataMeninggal' => $dataMeninggal, 'jorongName' => $jorongFilter])
        @else
            @foreach($dataPerJorong as $jorongName => $items)
                <div class="jorong-section">
                    ...
                    @include('operator.laporan-meninggal._table', ['dataMeninggal' => $items])
                </div>
            @endforeach
        @endif
    @else
