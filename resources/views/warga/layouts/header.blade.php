<header class="topbar">
    ...
    <div class="user-menu">
        ...
        <a href="{{ route('warga.profil') }}" class="profile" style="text-decoration: none; color: inherit;">
            <img
            src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=0ea5e9&color=fff"
            alt="Profile"
            />
            <div class="profile-info">
            <h4>{{ ucwords(strtolower(auth()->user()->name)) }}</h4>
            <p>{{ ucwords(strtolower(auth()->user()->nik)) }}</p>
            </div>
            <i
